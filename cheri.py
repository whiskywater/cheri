...
pyc_file_path = 'input.pyc'

# Open the .pyc file and read the bytecode
with open(pyc_file_path, 'rb') as file:
    # Skip the header of the .pyc file that contains the magic number and timestamp
    file.read(16)  # This might need to be adjusted depending on the Python version
    # Load the code object from the .pyc file
    code_obj = marshal.load(file)

# Now disassemble the code object
with open("input.txt", "w") as output_file:
    dis.dis(code_obj, file=output_file)
...
if __name__ == "__main__":
    ...
    with open(r"default.txt", "r") as f:
    ...
    with open("input.txt", "r") as file:
        with open ("prompt.txt", "r") as file2:
            message = {file2.read()}, {file.read()}
            memory.addUser(username, message)

    status, name, response = asyncio.run(CHATAI.getChat(memory))
    if status:
        print(f"Bytecode Analysis: {response}")
